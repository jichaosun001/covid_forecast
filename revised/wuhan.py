...
def reform_list(clinical_case,all_case,incre_list,start=23,date=0):
    ...
    """
    incre_list_new=[]
    suspect=[2606,3260,3182,1957,3230,2622, 2073, 2067, 2272, 1814, 1685, 1400, 1225, 1135]
    suspect_sub=suspect[date:date+12]
    ...
    for i,number in enumerate(incre_list):
        if i>=start+date-11 and i<=start+date:
            if i<start+date:
...
def adjust_number_list(number_list,clinical_case_list=[12364,1667,922],all_case_list=[13436,2997,1923],clinical_date_list=[23,24,25]):
    """
    para start_number为截止该天 其中确诊病例出现12号对应27
    ...
    for i in range(0,len(number_list)):
        ...
        if incre_list[i]==892: ###对892按过去7天做拆解  
            chai_list_p=[1,2,3,4,5,6,8]
            chai_sum=sum(chai_list_p)
            chai_number=[round(i*892/chai_sum) for i in chai_list_p]
            for j in range(7):
                if j==6:
                    incre_list[i-6+j]=chai_number[j]
                else:
                    incre_list[i-6+j]=chai_number[j]+incre_list[i-6+j]
            new_number_list=incre_list_to_numbers(incre_list)
...
# with open(os.path.join(output_dir,'wuhan/adjust_number.json'),'w',encoding='utf-8') as w:
...
def plot_certain_date(plot_date,raw_number_list=raw_number_list):
    ...
    plt.savefig(os.path.join(output_dir,'wuhan/wuhan_increase_{}.png'.format(the_date)),dpi=500,bbox_inches = 'tight')
    plt.show()


    with open(os.path.join(output_dir,'wuhan/wuhan_pred_{}.csv'.format(all_dates[start_number])),'w',encoding='utf-8') as w:
    ...
    with open(os.path.join(output_dir,'wuhan/wuhan_parameter_{}.csv'.format(all_dates[start_number])),'w',encoding='utf-8') as w:
